...
export const anatomyOfACertificate: Step = {
  ...
  glossary: lessonTerms("certificate", "claim"),
  title: "Certificates — Identity Cards",
  wireContext: {
    where: "The client has received a certificate whose binding it must evaluate.",
    now: "The annotated fields expose the name, issuer, public key, validity, and signature.",
    why: "The client needs to know what is asserted before validating the certificate and peer key possession.",
  },
  prose:
    "<p>The bank's certificate is like a laminated ID card that says: \"this public key is valid for this name.\" The technical reality is narrower than the metaphor: ordinary domain-validated TLS certificates record an issuer-validated binding to a DNS name such as example.com. Domain validation does not establish a company's legal identity, office address, or trustworthiness. Anyone could print a fake laminated card claiming to be anyone — a plastic card by itself proves nothing. What matters is what's printed on it, and, as the next step covers, whose stamp is on the back.</p>" +
  ...
  bullets: [
    ...
    "Signature: protects the signed certificate fields; validation determines whether to trust the binding",
  ],
  takeaway: "A certificate binds a key to names or attributes; validation and proof of key possession establish peer trust.",
  textBlock: {
    lang: "text",
    lines: certLines,
    annotations: [
      { line: 0, label: "Subject", description: "The subject named in the certificate; modern TLS hostname validation checks SAN, not Subject CN alone.", colorClass: "c-rand" },
