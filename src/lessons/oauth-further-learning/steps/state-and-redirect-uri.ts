...
export const stateAndRedirectUri: Step = {
  ...
  prose:
    ...
    "<p>The booth also only ever mails your claim stub back to an address you registered with it in advance: the <strong>redirect_uri</strong>. It has to match exactly — not \"close enough\" — or the booth refuses to send anything there. Otherwise an attacker could register a lookalike address and have your claim stub — the authorization code — delivered straight to them.</p>" +
  ...
  bullets: [
    ...
    "redirect_uri: must match a pre-registered value exactly — stops the claim stub (authorization code) being delivered somewhere else",
  ...
  sequence: {
    ...
    messages: [
      ...
      { from: "as", to: "client", label: "Redirect back with claim stub (code) + state" },
