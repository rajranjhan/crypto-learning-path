...
export const signaturesAndWallets: Step = {
  ...
  title: "Signatures Let Public Networks Verify Spending",
  prose:
    "<p>In the notebook, a wallet is not a purse full of coins. It is a pen that can make your unique signature. When you spend, you write a payment note and sign that exact note with the relevant private key. Other nodes check the signature with the corresponding public key or address-derived commitment, then check the ledger rules: the input exists, it has not already been spent, and the transaction satisfies the spending conditions.</p>" +
    "<p>This is why blockchains use digital signatures rather than shared-secret MACs. The people checking the notebook are strangers across the internet; they cannot all share a secret with every spender. A signature lets anyone verify authorization while keeping the signing key private. Lose the pen, and you lose the ability to sign future spends. Let someone copy it, and they can sign as you.</p>",
  bullets: [
    "Wallets manage signing keys; the chain records transactions and ownership state",
